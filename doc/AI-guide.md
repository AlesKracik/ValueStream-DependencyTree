...
- **Optimistic Concurrency Control**: Every persisted entity (`customers`, `workItems`, `issues`, `teams`, `sprints`, `valueStreams`) carries `_version: number`. Every mutation endpoint requires the client's last-seen version and bumps it on success; mismatch returns 409 with the server's current document so the client can merge and retry. Field-level `PATCH /api/entity/:collection/:id` and array-element endpoints (`/items/:arrayPath`) let concurrent edits to different fields or different array elements coexist without clobbering. See [Concurrency](CONCURRENCY.md) for the full contract, merge semantics, and rollout history.
